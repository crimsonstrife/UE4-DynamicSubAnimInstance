...
#include "AnimGraphNode_DynamicSubInstance.generated.h"

UCLASS(MinimalAPI, HideCategories = ("Settings"))
class UAnimGraphNode_DynamicSubInstance : public UAnimGraphNode_Base
{
	GENERATED_BODY()
	...
	public:
		...
		// Gets the property on InOwnerInputClass that corresponds to InInputPin
		void GetInstancePinProperty(const UClass* InOwnerInputClass, UEdGraphPin* InInputPin, UProperty*& OutProperty);
	...
	private:
		...
		*/
		...
		static bool HasInstanceLoop_Recursive(UAnimGraphNode_DynamicSubInstance* CurrNode, TArray<FGuid>& VisitedNodes, TArray<FGuid>& NodeStack);
		...
		void OnInputClassChanged(IDetailLayoutBuilder* DetailBuilder);
		...
		void OnSetInstanceBlueprint(const FAssetData& AssetData, TSharedRef<IPropertyHandle> InputClassPropHandle);
		// ----- END UI CALLBACKS ----- //
};